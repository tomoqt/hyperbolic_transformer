...
ANALYSIS_BATCHES="${ANALYSIS_BATCHES:-8}"
ANALYSIS_BATCH_SIZE="${ANALYSIS_BATCH_SIZE:-8}"
ANALYSIS_MAX_TOKENS="${ANALYSIS_MAX_TOKENS:-4096}"
RUN_ROOT="${RUN_ROOT:-runs/shakespeare_isotropy}"
ANALYSIS_ROOT="${ANALYSIS_ROOT:-analysis_out/shakespeare_isotropy}"
...
run_train() {
    ...
    "$PYTHON_BIN" train.py config/train_shakespeare_char.py "${train_args[@]}" | tee "$log_file"
