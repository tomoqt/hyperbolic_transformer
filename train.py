...
def get_batch(split):
    ...
    if not predict_ahead:
        ...
        shifts = np.clip(shifts, 1, max_shift)
        
        # determine the maximum possible starting index
        max_start_idx = len(data) - (block_size - 1) - max_shift
        ...
        for i, shift in zip(ix, shifts):
            ...
            y_full = np.concatenate(([-1], y_seq)).astype(np.int64)
            
            x_list.append(torch.from_numpy(x_full))
            y_list.append(torch.from_numpy(y_full))
            
        x = torch.stack(x_list)
        y = torch.stack(y_list)

    # Filter out invalid tokens by replacing them with padding token 0
    # This part seems to have been removed in the original diff, but it's good practice to keep it
    # x = torch.where(x > meta_vocab_size, 0, x)
    # y = torch.where(y > meta_vocab_size, 0, y)
...
if os.path.exists(meta_path):
    ...
    if predict_ahead:
        # ensure vocab_size is large enough for shift tokens
        effective_vocab_size = meta_vocab_size + max_shift
        print(f"using multi-token prediction, effective vocab_size is {effective_vocab_size}")

# model init
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  bias=bias, vocab_size=None, dropout=dropout,
                  predict_ahead=predict_ahead, max_shift=max_shift, label_shift_prob=label_shift_prob) # start with model_args from command line
if init_from == 'scratch':
    ...
    if predict_ahead:
        model_args['vocab_size'] += max_shift
